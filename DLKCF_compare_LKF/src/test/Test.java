...
public class Test {
	...
	public static void main(String[] args) {
	    ...
	    TrueSolutionCTM trueSolutionCTM= new TriangularTrueCTM(dt, dx,cells,distMeasure,Nsecs,0);//* For the last variable, 0 means no low quality sensors and 1 means low quality sensors exist.
	    
	    TrueSolution[] trueSolution=new TrueSolution[Nsecs];
	    for(int i=0;i<Nsecs;i++){
	    	trueSolution[i] = new TriangularTrue(trueSolutionCTM, dt, dx, cellsec, overlapsec, i, Nsecs,1,0,false);//* The last three variables sequentially stand for existence of model parameters perturbation (0 for false and 1 for true), low quality agents (0 for false and 1 for true) and independent agents    
	    }
	    if (Nsecs==1){
	    ...
	    for(int i=0;i<Nsecs1;i++){
	    	trueSolution1[i] = new TriangularTrue(trueSolutionCTM, dt, dx, cellsec1, overlapsec1, i, Nsecs1,1,0,false);
	    }
	    if (Nsecs1==1){
	    ...
	    for(int i=0;i<Nsecs2;i++){
	    	trueSolution2[i] = new TriangularTrue(trueSolutionCTM, dt, dx, cellsec2, overlapsec2, i, Nsecs2,1,0,true);
	    }
	    if (Nsecs2==1){
	    ...
	    Estimation.exportResult(trueSolutionCTM, trueSolution,trueSolution1,trueSolution2, 2000,"160708");
	}
